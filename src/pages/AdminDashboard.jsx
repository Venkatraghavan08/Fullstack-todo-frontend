...
export default function AdminDashboard({ token }) {
  ...
  const fetchAll = async () => {
    try {
      ...
      const t = await fetch(`${API_BASE}/api/tasks/me`, { headers });
      const tData = await t.json();
      if (!t.ok) throw new Error(tData.error || 'Failed tasks');
      setTasks(tData);

      const r = await fetch(`${API_BASE}/api/requests`, { headers });
      const rData = await r.json();
      if (!r.ok) throw new Error(rData.error || 'Failed requests');
      setRequests(rData);

      const map = new Map();
      tData.forEach(x => { if (x.assignedTo) map.set(x.assignedTo._id, x.assignedTo.username); });
      rData.forEach(x => { if (x.createdBy) map.set(x.createdBy._id || x.createdBy, x.createdBy.username || x.createdBy); });
      setUsers(Array.from(map.entries()).map(([id, username]) => ({ id, username })));
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  const assignTask = async e => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/tasks`, {
        method: 'POST',
        headers,
        body: JSON.stringify({ description: desc, assignedTo })
      });
  ...
  const approve = async id => {
    ...
    if (res.ok) setRequests(p => p.map(r => r._id === id ? { ...r, status: 'approved' } : r));
  };

  return (
    <div className='card'>
      <h2>Admin Dashboard</h2>
      {loading && <div className='badge progress'>Loading...</div>}
          ...
          <div key={t._id} className='item'>
            <div>
              ...
              <div className='actions'>
                ...
                <small>Assigned to: {t.assignedTo?.username || 'unknown'}</small>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className='list'>
        {requests.map(r => (
          <div key={r._id} className='item'>
            ...
            {r.status === 'pending' &&
              <button className='ghost' onClick={() => approve(r._id)}>Approve</button>}
          </div>
        ))}
      </div>
    </div>
