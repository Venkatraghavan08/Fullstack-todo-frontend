...
export default function AdminDashboard({ token }){
  ...
  const fetchAll=async()=>{try{setLoading(true);setError(null);
    const t=await fetch('http://localhost:5000/api/tasks/me',{headers:{Authorization:`Bearer ${token}`}});const tData=await t.json();if(!t.ok) throw new Error(tData.error||'Failed tasks');setTasks(tData);
    const r=await fetch('http://localhost:5000/api/requests',{headers:{Authorization:`Bearer ${token}`}});const rData=await r.json();if(!r.ok) throw new Error(rData.error||'Failed requests');setRequests(rData);
  ...
  useEffect(()=>{fetchAll();},[]);
  const assignTask=async e=>{e.preventDefault();try{const res=await fetch('http://localhost:5000/api/tasks',{method:'POST',headers,body:JSON.stringify({description:desc,assignedTo})});const data=await res.json();if(!res.ok) throw new Error(data.error||'Failed to assign');setDesc('');setAssignedTo('');setTasks(p=>[data,...p]);}catch(e){alert(e.message);}}
  const approve=async id=>{const res=await fetch(`http://localhost:5000/api/requests/${id}`,{method:'PUT',headers,body:JSON.stringify({status:'approved'})});if(res.ok) setRequests(p=>p.map(r=>r._id===id?{...r,status:'approved'}:r));}
